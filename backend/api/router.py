...
本模块使用 FastAPI Depends 进行依赖注入，避免全局可变状态。
"""
...
def get_db() -> DatabaseManager:
    ...
    if _db_manager is None:
        raise RuntimeError("DatabaseManager 未初始化，请确保已调用 set_dependencies")
    return _db_manager


def get_rag() -> RAGEngine:
    """FastAPI Depends 依赖：获取 RAG 引擎"""
...
def get_agent() -> AgentEngine:
    """FastAPI Depends 依赖：获取 Agent 引擎"""
...
async def clear_conversation(
    ...
    try:
        ...
        return APIResponse(code=200, msg="对话历史已清空")
    except Exception as e:
